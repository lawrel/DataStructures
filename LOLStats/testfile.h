...
//These first four functions all perform the job of a function of algorithm that would use iterators
//since we cannot use find, I had to make separate functions for vectors of classes annd vectors of strings
...
void buildLists(int flag,string champion,string player,vector<string> &player_names,
	...
	players[findIndexPLAYER(players,player)].addChampion(champion); //adds current champion to list the player has used
	champions[findIndexCHAMP(champions,champion)].addWinOrLoss(flag); //adds win or loss dependent on flag
}
void parse_file2(ifstream& infile, vector<PLAYER> &players, vector<CHAMPION> &champions, int flag1,int flag2)
void parse_file(ifstream& infile, vector<PLAYER> &players, vector<CHAMPION> &champions) {
	...
	while (infile>>player) {
		...
		if (player=="WINNING") { //case 1: WINNING TEAM appears first
			infile>>temp>>player;
			while (player!="LOSING"&&player!="EVENTS") {
			infile>>temp>>temp2>>champion;
			buildLists(1,champion,player,player_names,champion_names,players,champions);
			infile>>player;
			} 
			if (player=="LOSING") {
				infile>>temp>>player;
				while (player!="WINNING"&&player!="EVENTS") {
				infile>>temp>>temp2>>champion;
				buildLists(0,champion,player,player_names,champion_names,players,champions);
				infile>>player;
				}
			}
			if (player=="EVENTS") {
		...
		if (player=="LOSING") {
			...
			}                          //SAME AS ABOVE, JUST PASSES FLAG VARIABLE IN REVERSE ORDER
			if (player=="EVENTS") {
				infile>>temp;
				while(temp!="END") {
					int minion = 1;
